...
export default function CoachSpotlight({ initialCoaches }: { initialCoaches: any[] }) {
    const DEFAULT_COACH = {
        name: "Skip Holtz",
        role: "Head Coach",
        headshot_url: "https://cdn.discordapp.com/attachments/723741657048285275/1447010738735546467/aHR0cHM6Ly9zdG9yYWdlLmdvb2dsZWFwaXMuY29tL3VmbGVhZ3VlLXByb2QvMjAyNS8wMS8yOS9jNzk4OWUzYy00NmE3LTQ5MWEtYWNlZC01ODNkMjk3YWYyNTAud2VicA.png?ex=69361144&is=6934bfc4&hm=35e9f97c0a40e11c7024392cb3e13123215b9b1c6674d07c2d5f1415caf4f29e&",
        tagline: "Leading the Stallions’ modern dynasty with discipline, detail, and relentless preparation."
    };

    const coachesToRender = (initialCoaches && initialCoaches.length > 0) ? initialCoaches : [DEFAULT_COACH];
                        ...
                        <div className="mx-auto max-w-6xl px-6 py-6 lg:px-10 lg:py-8 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between relative z-10">
                            ...
                            <div className="flex flex-col items-center lg:items-start text-center lg:text-left w-full lg:w-auto">
                                    ...
                                    <div className="relative flex-shrink-0">
                                        ...
                                        <div className="relative h-20 w-20 rounded-full border-2 border-[#C5B783] shadow-lg overflow-hidden bg-black/50">
                                            {coach.headshot_url ? (
                                                ...
                                                <div className="w-full h-full bg-neutral-800 flex items-center justify-center text-xs text-neutral-500">IMG</div>
                                            )}
                                        </div>
                                    </div>
                                ...
                                <p className="text-white/60 text-lg font-medium leading-relaxed max-w-2xl">
                                    {coach.tagline || coach.bio_short || "Birmingham Stallions Coaching Staff"}
                                </p>
                            </div>

                            {/* Right: Big Stats - Only show for Head Coach (conventionally first) or if it's the default */}
                            {(index === 0 || coach.role.toLowerCase().includes('head')) && (
