...
import { updateLeagueDetails, regenerateLeagueCode, updateMaxTeams, deleteTeam, startLeague, resetLeague, archiveLeague, deleteLeague } from '../../../actions';
import { Lock, Eye, RefreshCw, AlertTriangle, Trash2, Archive, Save, X } from 'lucide-react';

interface SettingsClientProps {
    league: any;
    teams: any[];
    hasStarted: boolean; // Derived from weeks existence
    userId: string;
}

export default function SettingsClient({ league, teams, hasStarted, userId }: SettingsClientProps) {
    ...
    const handleUpdateMaxTeams = async (newVal: number) => {
        setLoading(true);
        try {
            await updateMaxTeams(league.id, newVal);
            setMaxTeams(newVal);
            alert('Max teams updated!');
        } catch (err: any) {
            alert(err.message);
        } finally {
            setLoading(false);
    ...
    const handleReset = async () => {
        ...
        // Ideally prompt for text match, but standard confirm is okay for MVP quick actions if explicit enough
        setLoading(true);
        try {
    ...
    return (
        <div className="space-y-12 max-w-4xl mx-auto">
            ...
            <section className="space-y-6">
                <h2 className="text-2xl font-black italic uppercase text-white border-b border-white/10 pb-2">League Settings</h2>

                <form onSubmit={handleSaveDetails} className="bg-neutral-900 border border-white/10 rounded-xl p-6 space-y-6">
                    <div>
                    ...
                    <div className="flex justify-end">
                        <button type="submit" disabled={loading} className="bg-white text-black font-bold uppercase px-6 py-2 rounded hover:bg-gray-200 disabled:opacity-50">
            ...
            <section className="space-y-6">
                <div className="flex items-center justify-between border-b border-white/10 pb-2">
                    <h2 className="text-2xl font-black italic uppercase text-white">Team Management</h2>
                ...
                <div className="bg-neutral-900 border border-white/10 rounded-xl p-6 space-y-8">
                    {/* Max Teams */}
                    <div>
                        <label className="block text-sm font-bold text-gray-400 uppercase mb-2">Max Teams</label>
                        <select
                            value={maxTeams}
                            disabled={hasStarted || loading}
                            onChange={(e) => handleUpdateMaxTeams(parseInt(e.target.value))}
                        ...
                        <p className="text-xs text-gray-500 mt-2">Cannot be lower than current team count.</p>
                    </div>
            ...
            {/* 3. SEASON CONTROL */}
            <section className="space-y-6">
                <h2 className="text-2xl font-black italic uppercase text-white border-b border-white/10 pb-2">Season Control</h2>
